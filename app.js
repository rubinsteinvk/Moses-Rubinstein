...
const style=document.createElement('style');style.textContent='.word.active-word{background:#e8d99d!important;border-radius:3px;box-shadow:0 0 0 1px rgba(130,100,40,.15)}.ru .word.active-word{background:#d9c7aa!important}';document.head.appendChild(style);
const scenes=[
...
function render(n){
  ...
  text.innerHTML=`<h2 class="scene-heading">${esc(s.title)}</h2><div class="sync-hint">Синхронизация поющего текста: включается кнопкой «Подсветка». Немецкое слово и его русский эквивалент подсвечиваются одновременно.</div>`+
    s.lines.map(l=>{const start=globalIndex;const words=l.words?l.words.map(w=>{const j=globalIndex++;return `<span class="word de-word" data-word="${j}">${esc(w.de)}</span>`}).join(' '):esc(l.de);const ru=l.words?l.words.map((w,j)=>`<span class="word ru-word" data-word="${start+j}">${esc(w.ru)}</span>`).join(' '):esc(l.ru);return `<div class="line"><div class="de">${words}</div><div class="ru">${ru}</div></div>${l.note?`<div class="note">${esc(l.note)}</div>`:''}`}).join('');
  clearHighlight();
}
...
function syncWords(t){clearHighlight();if(!syncEnabled||!timing.length)return;const x=timing.find(v=>t>=v.start&&t<v.end);if(x)document.querySelectorAll(`[data-word="${x.index}"]`).forEach(e=>e.classList.add('active-word'));}
window.syncMosesWords=syncWords;
function tick(){if(ytPlayer&&ytPlayer.getPlayerState&&ytPlayer.getPlayerState()===YT.PlayerState.PLAYING){syncWords(ytPlayer.getCurrentTime());rafId=requestAnimationFrame(tick);}else{rafId=null;}}
function startTick(){if(!rafId)tick();}
window.onYouTubeIframeAPIReady=()=>{ytPlayer=new YT.Player('ytPlayer',{events:{onReady:()=>{syncWords(ytPlayer.getCurrentTime());},onStateChange:e=>{if(e.data===YT.PlayerState.PLAYING)startTick();else if(e.data===YT.PlayerState.PAUSED||e.data===YT.PlayerState.ENDED||e.data===YT.PlayerState.CUED){if(rafId)cancelAnimationFrame(rafId);rafId=null;syncWords(ytPlayer.getCurrentTime());}},onError:e=>console.warn('YouTube player error',e.data)}})};
Promise.all([fetch('data/picture5.json').then(r=>r.json()),fetch('data/picture5-timing.json').then(r=>r.json())]).then(([d,t])=>{scenes[4].lines=d.lines.map(x=>({words:x.de.map((de,i)=>({de,ru:x.ru[i]||''}))}));timing=(t.words||[]).filter(v=>Number.isFinite(v.start)&&Number.isFinite(v.end));const count=document.getElementById('timingCount');if(count)count.textContent=`Временные метки: ${timing.length} из ${t.authoritative_word_count||97} слов`;if(+mobile.value===4)render(4);}).catch(err=>console.error(err));
const syncButton=document.getElementById('syncToggle');
function updateSyncButton(){syncButton.textContent=syncEnabled?'◉ Подсветка: ВКЛ':'○ Подсветка: ВЫКЛ';syncButton.setAttribute('aria-pressed',String(syncEnabled));syncButton.classList.toggle('sync-on',syncEnabled);syncButton.classList.toggle('sync-off',!syncEnabled);if(!syncEnabled)clearHighlight();else if(ytPlayer&&ytPlayer.getCurrentTime)syncWords(ytPlayer.getCurrentTime());}
