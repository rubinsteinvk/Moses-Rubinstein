...
function render(n){const s=scenes[n];document.querySelectorAll('.scene').forEach((e,i)=>e.classList.toggle('active',i===n));mobile.value=n;text.innerHTML=`<h2 class="scene-heading">${esc(s.title)}</h2><div class="sync-hint">Синхронизация использует выбранные word-level метки. Для слов без надёжной метки подсветка не включается.</div>`+s.lines.map(l=>`<div class="line"><div class="de">${l.words?l.words.map((w,j)=>`<span class="word de-word" data-word="${j}">${esc(w.de)}</span>`).join(' '):esc(l.de)}</div><div class="ru">${l.words?l.words.map((w,j)=>`<span class="word ru-word" data-word="${j}">${esc(w.ru)}</span>`).join(' '):esc(l.ru)}</div></div>${l.note?`<div class="note">${esc(l.note)}</div>`:''}`).join('')}
...
Promise.all([fetch('data/picture5.json').then(r=>r.json()),fetch('data/picture5-timing.json').then(r=>r.json())]).then(([d,t])=>{scenes[4].lines=d.lines.map(x=>({words:x.de.map((de,i)=>({de,ru:x.ru[i]||''}))}));timing=(t.words||[]).filter(v=>Number.isFinite(v.start)&&Number.isFinite(v.end));if(+mobile.value===4)render(4)}).catch(err=>console.error(err));
...
let ytPlayer=null;
window.onYouTubeIframeAPIReady=()=>{ytPlayer=new YT.Player('ytPlayer',{events:{onStateChange:e=>{if(e.data===YT.PlayerState.PLAYING)requestAnimationFrame(tick)}}})};
function tick(){if(ytPlayer&&ytPlayer.getPlayerState()===YT.PlayerState.PLAYING){syncWords(ytPlayer.getCurrentTime());requestAnimationFrame(tick)}}
